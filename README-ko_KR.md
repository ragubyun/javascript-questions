...
JavaScript 의 이벤트 큐 때문에 `setTimeout` 함수는 반복문이 실행된 _후에_ 호출됩니다. 첫 번째 반복문의 변수 `i`는 `var` 키워드로 선언되었기 때문에 전역 값입니다. 반복문이 실행되는 동안 단항연산자 `++`를 사용하여 `i`의 값을 `1`씩 증가시켰습니다. 첫번째 예제에서는 `setTimeout` 함수가 호출될 때 `i`는 `3`입니다.

In the second loop, the variable `i` was declared using the `let` keyword: variables declared with the `let` (and `const`) keyword are block-scoped (a block is anything between `{ }`). During each iteration, `i` will have a new value, and each value is scoped inside the loop.
두 번째 반복문에서 변수 `i`는 `let` 키워드로 선언되었습니다. `let`(and `const`) 키워드로 선언된 변수들은 블록 범위입니다(블록은 `{ }` 사이에 있는 어떤 것). 반복 과정에서 `i`는 새 값을 가지며 각 값은 반복문 내부 범위에 있습니다.
...
`diameter`은 일반 함수이고,`perimeter`은 arrow 함수입니다.

With arrow functions, the `this` keyword refers to its current surrounding scope, unlike regular functions! This means that when we call `perimeter`, it doesn't refer to the shape object, but to its surrounding scope (window for example).
arrow 함수를 사용하면 `this` 키워드는 일반 함수와 달리 현재 주변 범위를 참조합니다! 즉, `perimeter`을 호출하면 shape 객체가 아니라 주변의 범위 (예를 들어 창?)를 참조합니다.


There is no value `radius` on that object, which returns `undefined`.
shape 객체에는 `undefined`를 반환하는 `radius` 값이 없습니다.
...
단항 더하기 연산자는 피연산자를 숫자로 변환하려고 시도합니다. `true`는 `1`이고 `false`는 `0`이다.

The string `'Lydia'` is a truthy value. What we're actually asking, is "is this truthy value falsy?". This returns `false`.
`'Lydia'`라는 문자열은 진리값 입니다. 우리가 실제로 요구하는 것은 "이 값이 참입니까?"입니다. false를 반환합니다.
...
JavaScript에서는 모든 객체 키가 문자열입니다(심볼이 아닌 경우). 문자열로 _type_을 지정할 수는 없지만, 항상 후드에서 문자열로 변환됩니다.

JavaScript interprets (or unboxes) statements. When we use bracket notation, it sees the first opening bracket `[` and keeps going until it finds the closing bracket `]`. Only then, it will evaluate the statement.
JavaScript는 문장을 해석(or unboxes)합니다. 괄호 표기법을 사용할 때 첫 번째 여는 괄호`[`를 보고 닫는 괄호`]`를 찾을 때까지 계속 진행합니다. 그래야만 문장을 평가할 것입니다.
